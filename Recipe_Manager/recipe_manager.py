...
class Recipe:
    ...
    # Function for saving the recipe to the file
...
class RecipeManager:
    def __init__(self):
        ...
        # This was to help us interate thought the dictionary in resipes.py
        # i = 0
    ...
    # # This seems to have created an infinite loop. 
    # We couldnt find a solution but we wanted to keep it in to show what we were trying to do.
    #     temp = {
    ...
    # # Updating function using index of the recipe to change the values of each variable            
    ...
    # Delete a recipe by its index
    def delete_recipe_index(self, recipe_index):
        if recipe_index >= 0 and recipe_index < len(self.recipes):
            temp = os.path.join(recipe_folder_path, f"{self.recipes[recipe_index].get_title()}.txt")
            os.remove(temp)
            del self.recipes[recipe_index]

    # Creates a recipes file and then saves the recipes to it in a txt file
    ...
    # This allos you to select a file to load a recipe from
    def load_recipe_from_file_selected(self, filename):
    ...
    # Loads recipes from a folder when you open the program
    def load_recipes_from_folder(self):
        files = os.listdir(recipe_folder_path)
        for file in files:
            if file.endswith('txt'):
                with open(os.path.join(recipe_folder_path, file)) as recipe_file:
                    lines = recipe_file.readlines()
                    if len(lines) == 6:
                        title = lines[0].strip()
                        ingredients = [ingredient.strip() for ingredient in lines[1].split(",")]
